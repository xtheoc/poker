...
export const CTM_NL2_LEARNING: StrategyLearningSystem = {
  ...
  lessons: [
    ...
    },
    {
      ...
      assets: [
        { id: "action-first", kind: "rewrite", title: "1. Did someone bet before you?", body: "Tiny bet: treat it as a check. You may raise a player who makes tiny donk bets often, but only with a made hand or a real draw.\n\nNormal bet: do not automatically bluff back. Continue with a made hand or a real draw. With nothing, fold.", sources: [book([130, 141], "Planning a hand and c-bet exceptions")] },
        { id: "reason-to-bet", kind: "rewrite", title: "2. They checked. Do you have a reason to bet?", body: "Multiway: bet only top pair or a strong draw. With air, check.\n\nHeads-up: bet for value when worse hands call. Semi-bluff when better hands may fold and you can improve on later cards. If neither is true, check.\n\nBefore you bet, name one worse hand that calls or one turn card that helps you. If you cannot, check.", sources: [book([130, 151], "Planning a hand and c-bet exceptions")] },
        { id: "cbet-sizes", kind: "rewrite", title: "3. Then choose the size", body: "55%: dry ace- or king-high board, you missed, two low cards, no flush draw. 60%: ordinary pressure when you decide to bluff.\n\n75%: a good hand against a sticky regular. 100%: top pair good kicker or better against a fish or SLP. 150%: a monster against someone who calls too much.\n\nSmall when bluffing dry boards. Bigger when a worse hand will call.", sources: [book([132, 151], "C-bet plans and sizing")] },
      ],
      mastery: [{ kind: "drill", drillId: "flop-plan", minimumRuns: 2, minimumAccuracy: 85 }],
      playbook: [{ id: "flop", heading: "Flop plan", takeaway: "1. Did someone bet before you? 2. If they checked, is it multiway? 3. Do worse hands call, or can better hands fold while you improve? 4. Only then choose the size.", sources: [book([130, 151], "Flop")]}],
    },
    {
      ...
      assets: [{ id: "response-tree", kind: "rewrite", title: "4. Read their response", body: "Normal raise: fold. At NL2, a real flop raise is value-heavy. Do not stack off one-pair hands.\n\nMin-raise: in position, call with middle pair or top pair. With air, fold.\n\nCall: look at fold-to-c-bet. 70% or more: their call is strong, so stop bluffing. 59% or less: they are sticky, so value bet your good hands again.", sources: [book([151, 168], "Donk bets, raises and floats")] }],
      mastery: [{ kind: "drill", drillId: "cbet-response", minimumRuns: 2, minimumAccuracy: 85 }],
      playbook: [{ id: "cbet-response", heading: "After you bet", takeaway: "Normal raise: fold. Min-raise: call IP with a pair, fold air. High-folder calls: stop bluffing. Sticky-player calls: value bet again.", sources: [book([151, 168], "After the c-bet")]}],
    },
    {
